 ...
 */
public class ConnectedHandler_1 extends Handler {
     ...
     */
    public ConnectedHandler_1(final Handler _under, int _localId, String _destination) {
    ...
    public void handle(Message message) {
        // Split the received message payload into its components.
        String[] parts = message.payload.split(";");

        try {
            // Parse the sender ID, destination ID, packet ID, and the message flag from the payload.
            ...
            // If the message is an initial HELLO message and the remote ID has not been set yet.
            if (flag.equals(HELLO)) {
                if (rc_destinationId == -1 && rc_packageID == 0 && rc_senderId >= 0) {
                    ...
                    synchronized (hello_mutex) {
                        // Notify any threads waiting for the initial HELLO message.
                        hello_mutex.notifyAll();
                    }
                    // Send an ACK in response to the HELLO message without using the send method
                    // because an ACK for an ACK is not expected.
                    String ackPayload = localId + ";" + remoteId + ";" + packetNumberSender + ";" + ACK;
            ...
            // If the message is an ACK.
                ...
                if (rc_senderId == remoteId && rc_destinationId == localId && rc_packageID == packetNumber) {
                    synchronized (mutex) {
                        // Notify any threads waiting for an ACK of the sent message.
                        mutex.notifyAll();
                    }
                    // Cancel the resend task if it's scheduled because the ACK has been received.
                    if (task != null) {
                        task.cancel();
                    }
                }
            ...
            // Other messages
           if(!flag.equals(HELLO) && !flag.equals(ACK)){
                // Validate that the message is from the expected sender and has the expected packet ID.
                    // Increment packetNumberSender to expect the next message.
                    packetNumberSender++;

                    // Construct the payload to send upwards.
                String sourceAddress = String.valueOf(rc_senderId); // This should probably be the sender's ID, not the destination ID.
                String actualPayload = flag.isEmpty() ? "" : flag;
                Message toSendUpwards = new Message(actualPayload, sourceAddress);

                // Pass the message to the upper layer if an aboveHandler exists.
                aboveHandler.receive(toSendUpwards);
            }
